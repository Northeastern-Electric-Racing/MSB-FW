...
int main(void)
{
  ...
  {
    device_loc = DEVICE_FRONT_RIGHT;
  }
  else if (!loc1 && loc2)
  {
    device_loc = DEVICE_BACK_LEFT;
  }
  else if (!loc1 && !loc3)
  {
    device_loc = DEVICE_BACK_RIGHT;
  }
  else
  {
    device_loc = DEVICE_FRONT_LEFT;
  ...
  #ifdef SENSOR_IMU
  imu_monitor_handle = osThreadNew(vIMUMonitor, NULL, &imu_monitor_attributes);
